            ...
            <div class="">
                <FormKit type="form" id="formulario" :actions="false" @submit="inciarSesion" :value="persona">
                    ...
                    <FormKit type="text" label="Documento" name="documento" placeholder="Número de documento"
                        v-model="persona.documento" validation="number|required|?length:10" :validation-messages="{
                            length: 'Documento invalido',
                            required: 'Documento es obligatorio',
                            number: 'No puedes ingresar letras'
                        }" />
                    <FormKit placeholder="Ingrese su contraseña" name="contrasena" type="password" label="Contraseña"
                        value="mySecretPassword!" prefix-icon="password" suffix-icon="eyeClosed"
                        @suffix-icon-click="handleIconClick" v-model="persona.contrasena" validation="required|?length:8"
                        :validation-messages="{
                    ...
                    <FormKit
                        ...
                        type="submit" label="Loguear" :errors="['Ingrese sus datos para continuar']" />
                </FormKit>

            </div>
            <button class=" justify-center" @click="registrarse">
                <p class=" cursor-pointer font-semibold text-lg text-gray-600 ">¿Aún no tienes una cuenta? <span
                        class="text-gray-800">Registrate</span></p>
            </button>
        </div>
    </div>
</template>
